    ...
    <div class="content">
      <ul v-loading.body="loading">
        <li v-for="(item, index) in msg" :key="item.id"
            ...
            :title="item.title"
            @click="click(item.id)">
          <i class="el-icon-arrow-right" :class="[index % 2 === 0?'i-white':'i-green']" style="font-weight: bold;"></i>
...
export default {
  ...
  mounted () {
    this.loading = true
    // 取cookie
    this.openSimple = !this.$cookie.get('bfma')
    let url = Host + 'bestFirstMathAbove'
    this.post(url, '', res => {
      this.loading = false
      if (res.status) {
        this.info = res.info
  ...
  methods: {
    ...
    bClose () {
      ...
      this.$cookie.set('bfma', 'Helloworld', parseInt(this.info.adInfo.interval))
    }
  }
}
</script>
